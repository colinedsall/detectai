...
class AIContentGenerator:
    def __init__(self):
        ...
        # Define project root for consistent paths
        self.project_root = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
        
        # Create output directory
        os.makedirs(os.path.join(self.project_root, 'training_data/ai'), exist_ok=True)
        ...
        # Try 'ai_generation' section first, then 'training'
        ai_gen_config = self.config.get('ai_generation', {})
        training_config = self.config.get('training', {})
        
        self.topics = ai_gen_config.get('ai_topics', []) or training_config.get('ai_topics', [])
        
        # Free AI text generation APIs
        hf_token = self.api_keys.get('HUGGINGFACE_API_KEY', '')
        self.apis = [
            {
                'name': 'huggingface',
                'url': 'https://router.huggingface.co/api-inference.huggingface.co/models/gpt2',
            ...
            #     'headers': {'api-key': self.api_keys.get('DEEPAI_API_KEY', '')},
            #     'method': 'post',
            #     'data_key': 'text',
            #     'response_key': 'output'
            # }
    ...
    def _load_api_keys(self) -> dict:
        ...
        keys = {}
        api_keys_path = os.path.join(self.project_root, '.api_keys')
        ...
        if os.path.exists(api_keys_path):
            try:
                ...
                print(f" Loaded API keys from {api_keys_path}")
            except Exception as e:
                print(f" Warning: Could not load API keys: {e}")
        else:
            print(f" Warning: No .api_keys file found at {api_keys_path}")
            print("   Create one with: HUGGINGFACE_API_KEY=hf_your_token_here")
    ...
    def generate_with_huggingface(self, prompt: str, max_length: int = 200) -> str:
        """Generate text using Hugging Face's Inference Providers API."""
        
        hf_token = self.api_keys.get('HUGGINGFACE_API_KEY', '')
        
        # Check if we have a valid token (not placeholder)
        if not hf_token or hf_token == 'hf_your_token_here' or not hf_token.startswith('hf_'):
            print(" No valid HuggingFace API key found, using fallback...")
        ...
        try:
            ...
            response = requests.post(
                ...
                json={
                    ...
                    'messages': [
                        {'role': 'user', 'content': f'Write a detailed article about: {prompt}'}
                    ],
            ...
            if response.status_code == 200:
                ...
                if 'choices' in result and len(result['choices']) > 0:
                    ...
                    if generated:
                        print(f" Generated {len(generated.split())} words via HuggingFace API")
                        return generated
            else:
    ...
    def generate_with_ollama(self, prompt: str, max_tokens: int = 500, model: str = "gpt-oss:20b") -> str:
        ...
        try:
            response = requests.post(
                ...
                timeout=120  # Local inference can take longer
            )
            ...
            if response.status_code == 200:
                ...
                if 'choices' in result and len(result['choices']) > 0:
                    generated = result['choices'][0].get('message', {}).get('content', '')
                    if generated:
                        print(f" Generated {len(generated.split())} words via Ollama ({model})")
                        return generated
            else:
    ...
    def generate_content_samples(self, num_samples: int = 50, target_words: int = 500) -> List[Dict]:
        ...
        if self.topics:
            ...
            # Fallback to hardcoded topics if config is empty
            topics = [
                # Technology & Science
    ...
    def save_samples(self, samples: List[Dict]):
        ...
        for sample in samples:
            ...
            filename = f"training_data/ai/ai_generated_{sample['id']:03d}_{safe_topic}.txt"
    ...
    def generate_summary_report(self, samples: List[Dict]):
        ...
        # Save report
        with open('training_data/ai_generation_report.json', 'w') as f:
        ...
        print(f"  Report saved: training_data/ai_generation_report.json")
...
def main():
    ...
    print(f" Check the 'training_data/ai/' directory for generated samples.")
    print(f"\n Next steps:")
