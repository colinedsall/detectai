...
class TrainingDataCollector:
    def __init__(self):
        ...
        self.data_dir = os.path.join(self.project_root, 'training_data')
    ...
    def create_training_dataset(self, human_results: dict, ai_samples: list) -> dict:
        ...
        os.makedirs(os.path.join(self.data_dir, 'human'), exist_ok=True)
        os.makedirs(os.path.join(self.data_dir, 'ai'), exist_ok=True)
        
        # Save human samples
        for i, article in enumerate(human_articles):
            filename = os.path.join(self.data_dir, 'human', f"human_{i+1:03d}_{article['site']}.txt")
        ...
        for i, article in enumerate(ai_articles):
            filename = os.path.join(self.data_dir, 'ai', f"ai_{i+1:03d}_{article['topic'].replace(' ', '_')}.txt")
            with open(filename, 'w', encoding='utf-8') as f:
    ...
    def create_auto_labels(self, dataset: dict):
        ...
        # Add human samples
        human_files = [f for f in os.listdir(os.path.join(self.data_dir, 'human')) if f.endswith('.txt')]
        for filename in human_files:
            labels[os.path.join(self.data_dir, 'human', filename)] = "human"
        
        # Add AI samples
        ai_files = [f for f in os.listdir(os.path.join(self.data_dir, 'ai')) if f.endswith('.txt')]
        for filename in ai_files:
            labels[os.path.join(self.data_dir, 'ai', filename)] = "ai"
        
        # Save labels
        with open(os.path.join(self.data_dir, 'auto_labels.json'), 'w') as f:
